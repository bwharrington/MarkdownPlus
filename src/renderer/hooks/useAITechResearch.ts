...
function parseBlueprintResponse(text: string): TechResearchBlueprint {
    const fromParsed = (parsed: Record<string, unknown>): TechResearchBlueprint => ({
...
function buildDiscoveryPrompt(topic: string, blueprint: TechResearchBlueprint): string {
...
function buildExtractionPrompt(topic: string, blueprint: TechResearchBlueprint, depthLevel: GoDeepDepthLevel, webContext?: string): string {
    ...
    const submitTechResearch = useCallback(async (
        ...
        try {
            ...
            if (hasSerper) {
                ...
                setTechResearchPhase('discovery');
                console.log('[TechResearch] Phase: discovery — generating search queries');
                ...
                console.log('[TechResearch] Discovery complete', {
                    elapsed: Date.now() - startTime,
                ...
                console.log('[TechResearch] Search complete', {
                    elapsed: Date.now() - startTime,
                ...
                if (allUrls.size > 0) {
                    ...
                    console.log('[TechResearch] Fetching complete', {
                        elapsed: Date.now() - startTime,
